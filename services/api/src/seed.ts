...
async function bootstrap() {
  ...
  try {
    ...
    const techCat = await listingCatRepo.save(
      listingCatRepo.create({ categoryName: 'Informatique & High-Tech' }),
    );
    const kidsCat = await listingCatRepo.save(
      listingCatRepo.create({ categoryName: 'Enfants & Famille' }),
    );
    ...
    const cultureEventCat = await eventCatRepo.save(
    ...
    const mockUsers = [
      ...
      },
      {
        ...
        password: 'Password123!',
        nbId: 'nb-downtown',
        role: UserRoleEnum.NEIGHBOURHOOD_REP,
      },
      {
        email: 'felix@nabor.fr',
        firstName: 'Félix',
        lastName: 'Moreau',
        password: 'Password123!',
        nbId: 'nb-marais',
    ...
    for (const u of mockUsers) {
      ...
      const seededUser = await userRepo.findOneOrFail({
        where: { email: u.email },
      });
    ...
    await swipeRepo.save([
      ...
      swipeRepo.create({
        swiperId: uOscar.id,
        swipedId: uLea.id,
        direction: SwipeDirectionEnum.DISLIKE,
      }),
    ]);
    ...
    await listingContentService.updateContent(uBob.id, l2.id, {
      body_html:
        '<p>Canapé lourd, 3ème étage sans ascenseur. Bières offertes !</p>',
    ...
    await listingContentService.updateContent(uGabriel.id, l5.id, {
      body_html:
        '<p>Mécanicien amateur, outillage complet. Devis avant intervention.</p>',
    ...
    await listingContentService.updateContent(uJulie.id, l8.id, {
      body_html:
        "<p>Bois massif, quelques traces d'usage. Dimensions sur demande.</p>",
    ...
    await listingContentService.updateContent(uKarim.id, l9.id, {
      body_html:
        '<p>Récupérés lors de mon propre déménagement, en bon état.</p>',
    ...
    await listingReportService.createReport(
      uOscar.id,
      l6.id,
      'Annonce suspecte, demande des informations personnelles avant tout échange.',
    );
    await listingReportService.createReport(
      uMathis.id,
      l9.id,
      'Annonce en doublon, déjà publiée dans un autre groupe.',
    );
    await listingReportService.createReport(
      uGabriel.id,
      l11.id,
      'Prix annoncé ne correspond pas à la description du service.',
    );
    ...
    await new contractModel({
      ...
      pdf: {
        gridfs_file_id: 'gridfs-seed-contract-1',
      ...
      listing_snapshot: {
        ...
        neighbourhood_name: 'Marais District',
      },
      signature: {
        canvas_b64: null,
    ...
    await new contractModel({
      ...
      pdf: {
        gridfs_file_id: 'gridfs-seed-contract-2',
        mimetype: 'application/pdf',
        size_bytes: 22100,
      },
      parties: {
        ...
        requester: {
          pg_user_id: uFelix.id,
          full_name: 'Félix Moreau',
          email: 'felix@nabor.fr',
        },
      },
      listing_snapshot: {
        ...
        price_cents: 2000,
        listing_type: 'offer',
        neighbourhood_name: 'Downtown Paris',
      },
      signature: {
        canvas_b64: null,
        totp_verified_at: new Date(),
    ...
    await new contractModel({
      ...
      type: 'contract',
      sha256_hash:
        'b5d4045c3f466fa91fe2cc6abe79232a1a57cdf104f7a26e716e0a1917894e00',
      pdf: {
        gridfs_file_id: 'gridfs-seed-contract-3',
        mimetype: 'application/pdf',
        size_bytes: 19800,
      },
      parties: {
        provider: {
          pg_user_id: uJulie.id,
          full_name: 'Julie Bonnet',
          email: 'julie@nabor.fr',
        },
        requester: {
          pg_user_id: uNora.id,
          full_name: 'Nora Perrin',
          email: 'nora@nabor.fr',
        },
      },
      listing_snapshot: {
        ...
        price_cents: 4000,
        listing_type: 'offer',
        neighbourhood_name: 'La Villette',
      },
      signature: {
        canvas_b64: null,
        totp_verified_at: new Date(),
    ...
    await eventContentService.updateContent(uAlice.id, e1.id, {
      body_html:
        '<h3>BBQ Communautaire</h3><p>Parc central. Apportez de quoi partager.</p>',
      location: {
    ...
    await eventContentService.updateContent(uEmma.id, e2.id, {
      body_html:
        "<p>Rendez-vous à l'entrée du parc à 9h. Gants et sacs fournis.</p>",
    ...
    await eventContentService.updateContent(uGabriel.id, e4.id, {
      ...
      location: {
        address: 'Place de la Villette, Paris',
        geocode: '48.866,2.356',
      },
    });
    ...
    await eventContentService.updateContent(uLea.id, e6.id, {
      body_html:
        '<p>Ambiance décontractée, boissons chacun apporte quelque chose.</p>',
    ...
    await eventReportService.createReport(
      uJulie.id,
      e5.id,
      "Lieu indiqué inexact, l'adresse ne correspond à aucun square.",
    );
    await eventReportService.createReport(
      uNora.id,
      e6.id,
    ...
    const incidents = [
      {
        ...
        description: 'Trou énorme chaussée principale.',
        severity: IncidentSeverityEnum.HIGH,
        status: IncidentStatusEnum.IN_PROGRESS,
        body: '~50cm large, 15cm profond. Risque crevaison.',
        hint: '12 rue principale',
      },
      {
        reporterId: uBob.id,
        assignedTo: uCharlie.id,
        nbId: 'nb-marais',
        title: 'Éclairage public défectueux',
        description: '3 lampadaires éteints rue des Lilas.',
        severity: IncidentSeverityEnum.MEDIUM,
        status: IncidentStatusEnum.OPEN,
        body: 'Depuis une semaine. Zone très sombre le soir.',
        hint: 'Rue des Lilas, entre n°5 et n°15',
      },
      {
        reporterId: uFelix.id,
        assignedTo: null,
        nbId: 'nb-marais',
        title: 'Dépôt sauvage',
        description: 'Déchets abandonnés au coin de la rue.',
        severity: IncidentSeverityEnum.LOW,
        status: IncidentStatusEnum.OPEN,
        body: 'Cartons, vieux meubles. Présent depuis 3 jours.',
        hint: 'Angle rue des Écoles / rue Pasteur',
      },
      {
        reporterId: uJulie.id,
        assignedTo: uDavid.id,
        nbId: 'nb-villette',
        title: 'Banc public cassé',
        description: 'Planche fendue, risque de blessure.',
        severity: IncidentSeverityEnum.LOW,
        status: IncidentStatusEnum.OPEN,
        body: 'Assise fendue sur toute la longueur.',
        hint: 'Allée centrale du parc de la Villette',
      },
      {
        reporterId: uKarim.id,
        assignedTo: uCharlie.id,
        nbId: 'nb-downtown',
        title: "Tags sur la façade de l'école",
        description: 'Graffitis apparus dans la nuit.',
        severity: IncidentSeverityEnum.MEDIUM,
        status: IncidentStatusEnum.IN_PROGRESS,
        body: 'Mur entier côté cour recouvert.',
        hint: 'École primaire, rue des Tilleuls',
      },
      {
        reporterId: uOscar.id,
        assignedTo: null,
        nbId: 'nb-marais',
        title: 'Nuisances sonores nocturnes',
        description: 'Bruit récurrent après 23h depuis un chantier.',
        severity: IncidentSeverityEnum.LOW,
        status: IncidentStatusEnum.OPEN,
        body: 'Plusieurs riverains concernés, tous les soirs cette semaine.',
        hint: 'Chantier rue de la Poterne',
      },
      {
        reporterId: uGabriel.id,
        assignedTo: uDavid.id,
        nbId: 'nb-villette',
        title: "Fuite d'eau importante",
        description: 'Eau qui ruisselle en continu depuis un regard.',
        severity: IncidentSeverityEnum.HIGH,
        status: IncidentStatusEnum.IN_PROGRESS,
        body: 'Chaussée glissante, risque de chute. Signalé à la mairie.',
        hint: 'Angle quai de la Loire / rue de Crimée',
      },
      {
        reporterId: uHelene.id,
        assignedTo: uCharlie.id,
        nbId: 'nb-downtown',
        title: "Jeux d'enfants endommagés",
        description: 'Toboggan fissuré au square principal.',
        severity: IncidentSeverityEnum.MEDIUM,
        status: IncidentStatusEnum.RESOLVED,
        body: 'Pièce remplacée par les services techniques.',
        hint: 'Square du centre ville, aire de jeux',
      },
      {
        reporterId: uIsmael.id,
        assignedTo: uDavid.id,
        nbId: 'nb-marais',
        title: 'Odeur de gaz suspecte',
        description: "Forte odeur près d'un immeuble résidentiel.",
        severity: IncidentSeverityEnum.CRITICAL,
        status: IncidentStatusEnum.IN_PROGRESS,
        body: 'Pompiers et GRDF prévenus, immeuble évacué par précaution.',
        hint: '8 rue des Archives',
      },
      {
        reporterId: uMathis.id,
        assignedTo: uDavid.id,
        nbId: 'nb-villette',
        title: 'Arbre tombé sur le trottoir',
        description: 'Grosse branche bloque le passage piéton.',
        severity: IncidentSeverityEnum.MEDIUM,
        status: IncidentStatusEnum.RESOLVED,
        body: 'Évacuée par les espaces verts le lendemain matin.',
      ...
      },
      {
        ...
        assignedTo: null,
        nbId: 'nb-marais',
        title: "Fuite d'eau en cave commune",
        description: "Infiltration visible au sous-sol de l'immeuble.",
        severity: IncidentSeverityEnum.HIGH,
        status: IncidentStatusEnum.OPEN,
        body: "Odeur d'humidité, murs qui noircissent.",
        hint: '22 rue Vieille du Temple, sous-sol',
      },
      {
        reporterId: uNora.id,
        assignedTo: uCharlie.id,
        nbId: 'nb-downtown',
        title: 'Balançoire cassée',
        description: 'Chaîne rompue sur une balançoire du square.',
        severity: IncidentSeverityEnum.LOW,
        status: IncidentStatusEnum.RESOLVED,
        body: 'Remplacée lors de la maintenance mensuelle.',
        hint: 'Square du centre ville, aire de jeux',
      },
    ];
    ...
    for (const inc of incidents) {
      ...
      await new incidentDocModel({
        pg_incident_id: saved.id,
        body: inc.body,
        photos: [],
        location_hint: inc.hint,
        created_at: new Date(),
        updated_at: new Date(),
        synced_at: new Date(),
      }).save();
    ...
    const p1 = await pollRepo.save(
      pollRepo.create({
        ...
        description: 'Quelle couleur pour les nouveaux bancs ?',
        creatorId: uEmma.id,
    ...
    const oVert = await pollOptRepo.save(
      pollOptRepo.create({ pollId: p1.id, label: 'Vert forêt' }),
    );
    const oBleu = await pollOptRepo.save(
      pollOptRepo.create({ pollId: p1.id, label: 'Bleu océan' }),
    );
    const oGris = await pollOptRepo.save(
      pollOptRepo.create({ pollId: p1.id, label: 'Gris anthracite' }),
    );
    await voteRepo.save(
      voteRepo.create({ userId: uAlice.id, optionId: oVert.id, weight: 1 }),
    );
    await voteRepo.save(
    ...
    const p2 = await pollRepo.save(
      pollRepo.create({
        ...
        description: 'Choisissez les activités qui vous intéressent.',
        creatorId: uCharlie.id,
        neighbourhoodId: 'nb-downtown',
        pollType: PollTypeEnum.MULTIPLE,
        startsAt: new Date(),
        endsAt: new Date(Date.now() + 14 * 86400000),
        isAnonymous: true,
      }),
    );
    const oYoga = await pollOptRepo.save(
      pollOptRepo.create({ pollId: p2.id, label: 'Yoga / Méditation' }),
    );
    const oTheatre = await pollOptRepo.save(
      pollOptRepo.create({ pollId: p2.id, label: 'Théâtre amateur' }),
    );
    const oPotager = await pollOptRepo.save(
      pollOptRepo.create({ pollId: p2.id, label: 'Potager collectif' }),
    );
    const oInformatique = await pollOptRepo.save(
      pollOptRepo.create({ pollId: p2.id, label: 'Atelier informatique' }),
    );
    await voteRepo.save(
      voteRepo.create({ userId: uAlice.id, optionId: oYoga.id, weight: 1 }),
    );
    await voteRepo.save(
      voteRepo.create({ userId: uAlice.id, optionId: oPotager.id, weight: 1 }),
    );
    await voteRepo.save(
      voteRepo.create({
        userId: uEmma.id,
        optionId: oInformatique.id,
        weight: 1,
      }),
    );
    await voteRepo.save(
    ...
    const p3 = await pollRepo.save(
      pollRepo.create({
        ...
        description: 'Quel équipement ajouter en priorité ?',
        creatorId: uIsmael.id,
        neighbourhoodId: 'nb-marais',
        pollType: PollTypeEnum.SINGLE,
        startsAt: new Date(),
    ...
    const oKiosque = await pollOptRepo.save(
      pollOptRepo.create({ pollId: p3.id, label: 'Kiosque à journaux' }),
    );
    const oFontaine = await pollOptRepo.save(
      pollOptRepo.create({ pollId: p3.id, label: 'Fontaine à eau' }),
    );
    const oBoiteLivres = await pollOptRepo.save(
      pollOptRepo.create({ pollId: p3.id, label: 'Boîte à livres' }),
    );
    await voteRepo.save(
      voteRepo.create({ userId: uBob.id, optionId: oFontaine.id, weight: 1 }),
    );
    await voteRepo.save(
      voteRepo.create({
        userId: uFelix.id,
        optionId: oBoiteLivres.id,
        weight: 1,
      }),
    );
    await voteRepo.save(
    ...
    const oRetro = await pollOptRepo.save(
      pollOptRepo.create({ pollId: p4.id, label: 'Décennies rétro' }),
    );
    const oCarnaval = await pollOptRepo.save(
      pollOptRepo.create({ pollId: p4.id, label: 'Carnaval' }),
    );
    const oGuinguette = await pollOptRepo.save(
      pollOptRepo.create({ pollId: p4.id, label: 'Guinguette' }),
    );
    await voteRepo.save(
      voteRepo.create({
        userId: uDavid.id,
        optionId: oGuinguette.id,
        weight: 1,
      }),
    );
    await voteRepo.save(
      voteRepo.create({ userId: uGabriel.id, optionId: oRetro.id, weight: 1 }),
    );
    await voteRepo.save(
      voteRepo.create({
        userId: uGabriel.id,
        optionId: oCarnaval.id,
        weight: 1,
      }),
    );
    await voteRepo.save(
    ...
    async function sendMessage(
      ...
      await new msgMongoModel({
        pg_message_id: msgId,
        pg_group_id: groupId,
        pg_sender_id: sender.id,
        content_encrypted: enc.encrypted,
        iv: enc.iv,
        auth_tag: enc.authTag,
        type: 'text',
        attachments: [],
        reactions: [],
        sent_at: new Date(atMs),
      }).save();
    }
    ...
    const neighbourhoodGroups: {
    ...
    }[] = [
      {
        ...
        description: 'Canal principal des résidents de Downtown.',
        nbId: 'nb-downtown',
        admin: uEmma,
        ...
        messages: [
          {
            sender: uEmma,
            content: 'Bienvenue à tous dans le groupe Downtown !',
          },
          {
            sender: uAlice,
            content: 'Merci Emma ! Qui est dispo pour le BBQ samedi prochain ?',
          },
          {
            sender: uCharlie,
            content: 'Je viendrai avec une salade et des boissons.',
          },
          {
            sender: uHelene,
            content: 'Je peux amener des chaises pliantes si besoin.',
          },
          {
            sender: uKarim,
            content:
              "Et moi le tournoi de pétanque du week-end d'après, qui est chaud ?",
          },
          {
            sender: uEmma,
            content: "Super ! Je m'occupe du matériel de cuisson pour le BBQ.",
          },
        ],
      },
      {
        name: 'Discussion générale - Marais',
        description: 'Canal principal des résidents du Marais.',
        nbId: 'nb-marais',
        admin: uBob,
        ...
        messages: [
          {
            sender: uBob,
            content: 'Salut le Marais, bienvenue dans le canal !',
          },
          {
            sender: uFelix,
            content: "Hello ! Quelqu'un a des nouvelles du nettoyage de rue ?",
          },
          {
            sender: uIsmael,
            content: "C'est prévu le mois prochain je crois.",
          },
          {
            sender: uLea,
            content: "Je peux relayer l'info sur le tableau d'affichage.",
          },
          { sender: uOscar, content: 'Merci Léa, tiens-nous au courant !' },
        ],
      },
      {
        name: 'Discussion générale - La Villette',
        description: 'Canal principal des résidents de La Villette.',
        nbId: 'nb-villette',
        admin: uDavid,
        ...
        messages: [
          {
            sender: uDavid,
            content: 'Bienvenue dans le canal de La Villette.',
          },
          { sender: uGabriel, content: "Merci David, content d'être là !" },
          {
            sender: uJulie,
            content: "Quelqu'un connaît un bon plombier dans le coin ?",
          },
    ...
    for (const g of neighbourhoodGroups) {
      ...
      const group = await chatGroupRepo.save(
      ...
      await uigRepo.save(
        g.members.map((m) =>
          uigRepo.create({
            ...
            groupId: group.id,
            roleInGroup:
              m.id === g.admin.id ? GroupRoleEnum.ADMIN : GroupRoleEnum.MESSAGE,
          }),
        ),
      );
    ...
    const dmTemplates: [string, string][] = [
      [
        ...
        'Hello ! Oui, hâte de faire connaissance.',
      ],
      [
        'Hey, tu connais un bon plombier dans le coin ?',
        'Oui je peux te filer un contact !',
      ],
      [
        "Merci encore pour hier, c'était top.",
        'Avec plaisir, on remet ça quand tu veux !',
      ],
      ['Tu vas au marché ce week-end ?', 'Oui, dimanche matin normalement.'],
      [
        "J'ai vu ton annonce, ça m'intéresse !",
        "Super, je t'envoie les détails.",
      ],
      ['Bienvenue dans le quartier !', 'Merci beaucoup, ça fait plaisir :)'],
      [
        'On se croise au parc parfois non ?',
        "Ah oui je crois t'avoir déjà vu !",
      ],
    ...
    for (let i = 0; i < seededUsers.length; i++) {
      ...
      const fr = await friendshipRepo.findOne({
        where: [
          { user1Id: a.id, user2Id: b.id },
          { user1Id: b.id, user2Id: a.id },
        ],
      });
    ...
    await new userMediaModel({
      pg_user_id: uAlice.id,
      type: 'avatar',
      data: Buffer.from('mock-avatar-alice'),
      mimetype: 'image/webp',
      size_bytes: 1024,
      width_px: 200,
      height_px: 200,
    ...
    await new userMediaModel({
      pg_user_id: uEmma.id,
      type: 'avatar',
      data: Buffer.from('mock-avatar-emma'),
      mimetype: 'image/webp',
      size_bytes: 1536,
      width_px: 200,
      height_px: 200,
