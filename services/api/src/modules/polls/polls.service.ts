...
export class PollsService {
  ...
  async updatePoll(
    ...
    const hasVotes = await this.voteRepo.count({
      where: { option: { pollId } },
    });
    if (hasVotes > 0)
  ...
  async addOption(
    ...
    if (hasVotes > 0)
      throw new ForbiddenException(
        "Impossible d'ajouter une option après le premier vote",
      );
  ...
  async deleteOption(
    ...
    if (hasVotes > 0)
      throw new ForbiddenException(
        'Impossible de supprimer une option avec des votes',
      );
  ...
  async vote(pollId: string, userId: string, optionId: string) {
    ...
    if (poll.endsAt && poll.endsAt < new Date())
      throw new ForbiddenException('Sondage terminé');
  ...
  private async attachResults(polls: Poll[]) {
    ...
    const anonymousPollIds = new Set(
      polls.filter((p) => p.isAnonymous).map((p) => p.id),
    );
    const voterUserIds = [
      ...new Set(
        votes
