...
export class AdminRgpdController {
  ...
  @ApiOkResponse({ description: 'Demandes RGPD retournées', type: [RgpdRequestDto] })
  ...
  async anonymize(@Param('user_id') userId: string): Promise<SuccessResponseDto> {
  ...
  @ApiForbiddenResponse({ description: 'Action réservée aux administrateurs' })
  @ApiNotFoundResponse({ description: 'Utilisateur introuvable' })
  async getStatus(@Param('user_id') userId: string): Promise<RgpdRequestStatusDto> {
