...
### Changed
- **Auto-update client Java Desktop — proxy GitHub Releases** : `GET /v1/updates/latest` proxifie désormais le manifeste `latest.json` de la dernière GitHub Release du client (cache en mémoire, TTL `UPDATES_CACHE_TTL_MS` défaut 5 min, repli sur le cache expiré si GitHub est injoignable, sinon `503`) et `GET /v1/updates/download` redirige (302) vers l'asset `nabor-desktop.zip` de la release. Nouvelles variables `UPDATES_GITHUB_REPO` (défaut `Mistereptil09/Nabor-Java-Client`) et `UPDATES_CACHE_TTL_MS`. Le schéma servi (`version`, `sha256`, `changelog_url`, `download_url`) correspond désormais exactement à ce que le client Java parse — auparavant le manifeste local placeholder (`jar_filename`, SHA-256 de zéros) ne pouvait déclencher aucune mise à jour et `/updates/download` servait un JAR alors que le client attend un bundle ZIP.
